...
import com.ncautomation.clock.dialogs.SelectAlarmDialog
import com.ncautomation.clock.extensions.*
import com.ncautomation.clock.helpers.*
import com.ncautomation.clock.models.*
import com.ncautomation.commons.dialogs.PermissionRequiredDialog
import com.ncautomation.commons.extensions.getDefaultAlarmSound
import com.ncautomation.commons.extensions.getFilenameFromUri
    ...
    private fun Intent.setNewAlarm() {
        ...
        val soundToUse = ringtone?.let {
            if (it == AlarmClock.VALUE_RINGTONE_SILENT) {
                AlarmSound(0, getString(com.ncautomation.commons.R.string.no_sound), SILENT)
            } else {
                try {
                    val uri = Uri.parse(it)
                    var filename = getFilenameFromUri(uri)
                    if (filename.isEmpty()) {
                        filename = getString(com.ncautomation.commons.R.string.alarm)
                    }
